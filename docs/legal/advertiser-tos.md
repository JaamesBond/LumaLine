...
impression or click **only after the 72-hour clawback window has elapsed** (see
§5). This is the "clawback-immune point" — the moment at which the spend is
final. We do not charge before this point.
...
balance) will result in serving being paused for the affected line item(s) until
...
**Pricing:** Billing is based on the bid you set at line item creation (CPVA:
per verified attention-second; CPC: per verified click where applicable). The
clearing price per delivered impression equals the cpva_bid at the time the
window was opened ("reserve-floor / first-price clearing"). Bids set after a
window opens do not affect that window's clearing price.

**Reconciliation:** We run daily reconciliation to ensure advertiser charges
equal ledger debits. In the event of a discrepancy, we will investigate and
correct within a reasonable period.
...
LumaLine** as the platform fee. This split applies to all cleared impression and
click revenue. The split is computed in integer micro-USD.
...
### 5.1 Clawback window
Every impression and click is initially **provisional**. A **72-hour clawback
window** runs from the time of the event. During this window, if we (or our
