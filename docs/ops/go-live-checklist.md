...
| 15 | ≥1 REAL advertiser: contract + KYC + creative + budget + bid>0 + payment method | ✅ LIVE + SERVING | **Advertiser "Degen" SEEDED 2026-07-03** (`is_house=false`, advertiser `4779db17…`): view-only CPVA (no landing URL, so the earlier Rickroll bait-and-switch concern is moot), bid **€0.01/view** (`cpva_bid_micros=10000`), budget **€5 / 3-day flight** (daily auto-derived), creative `"Share degeneracy with your friends fun and easy"` in `pending_review`; campaign + line_item in `draft`. **Nothing active → cannot serve or bill pre-swap.** **COMPLETE 2026-07-03:** (a) payment method attached — friend entered a live Visa ****5369 (customer `cus_Uokl8W…`, no default set → `choosePaymentMethod` first-attached picks it); (b) **ACTIVATED** — owner authorized; creative + line_item + campaign + advertiser all `active`. Degen now serves to logged-in publishers (weighted against the house creative, both weight 1), paced to ~€1.67/day. Anonymous users still get house-only. **First real charge (M5-T3) pending:** accrual → 72h clearing → `billing/charge` dry-run → owner-authorized real run → reconcile. |
...
Full ordered path (owner-vs-cc, verify per step): **`docs/ops/m5-go-live-plan.md`**.
