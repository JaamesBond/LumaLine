...
| 18 | **All edge fns current on remote** (added 2026-07-03) | ❌ | **`billing` is STALE** — remote v6 (2026-06-30, pre-M5): hardcodes the Stripe test card, no `/setup-link` (probe → 404). The merged M5 billing code was never redeployed. **Must redeploy before the swap** (`bash scratchpad/m5-deploy-billing.sh`; safe — test mode until swap). Others current: `lumaline-feed` v19, `auth-device` v11, `monitor` v3 (has `billing_stalled`), `stripe-connect` v9, `admin-booking` v7, `click` v13. |
